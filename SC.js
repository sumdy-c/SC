/**
 * @publishing
 *  Что-то, что возможно нужно сделать для публикации в массы.
 * 	1 - Создать 3 варианта SC:
 * 						  	- SC native
 * 						  	- SC jquery
 * 							- SC react (+ jsx) = вот это, что за монстр должен получиться ?
 * 2 - Типизация
 * 3 - Перенести SC правила в отдельный файл *.s1c, написать расширение для .vscode, на новое расширение файла
 * 4 - Все TODO, конечно
 * 5 - Для вариантов со сборкой, перенести настройки в scconfig.json
 * 6 - Первоначально создать следующий необходимый функционал:
 * 		- Проверка классов на странице ( тг избранное, ссылка на документацию )
 * 		- Автоматическое определение размера экрана пользователя, для подключения дальнейшей настройки-модуля @settingTool {fluid_adaptive}
 * 		- Темизация на основе правил - без изменения классов. @settingTool { SC_theme }
 * 		- Будет в TODO, потому что стыдно что нет, но отдельный класс Exeption быть должен.
 */

// TODO - продумать проверку классов на странице ( *полная информация по классам в консоле при флаге, помощь и подсказки, расширение SC )
